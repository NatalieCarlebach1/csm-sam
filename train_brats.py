...
def train_one_epoch(model, loader, optimizer, scheduler, loss_fn,
    ...
    model.train()
    totals = {"total": 0.0, "dice": 0.0, "bce": 0.0, "change": 0.0, "kl": 0.0}
    n_steps = 0
    ...
    for step, batch in enumerate(pbar):
        ...
        with autocast(enabled=cfg.training.amp):
            ...
            if mid_class_masks is not None and out["masks"].shape[1] == 4:
                ...
                kl = out.get("kl_loss")
                kl_term = (kl_beta_val * kl) if (kl is not None) else torch.tensor(0.0, device=device)
                losses = {
                    "total": mc_losses["total"] + kl_term,
                    "dice":  mc_losses["dice"],
                    "bce":   mc_losses["bce"],
...
def main():
    ...
    model = CSMSAM.from_pretrained(
        ...
        latent_retrieval_tau=float(getattr(cl_cfg, "retrieval_tau", 0.1)) if cl_cfg else 0.1,
